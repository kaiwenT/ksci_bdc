...
def cnn_model_fn(features, labels, mode):
    ...
    pool3_flat = tf.reshape(pool2, [-1, 1 * 105 * 128])
    # 全连接层FC1
    dense1 = tf.layers.dense(pool3_flat, units=256, activation=tf.nn.relu)
    # dropout1
...
def train():
    ...
    cnn_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir='G:/models/ksci_bdc_model/f111_cnn_1')
...
def predict(fname):
    ...
    active_users.to_csv('G:/dataset/a3d6_chusai_a_train/prediction/f111_cnn_1_' + fname + '.txt',
