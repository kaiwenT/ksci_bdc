...
def cnn_model_fn(features, labels, mode):
    ...
    pool3_flat = tf.reshape(conv2, [-1, 3 * 23 * 128])
    # 全连接层FC1
...
def train():
    ...
    cifar_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir='G:/models/ksci_bdc_model')
...
def predict():
    ...
    np.savetxt('G:/dataset/a3d6_chusai_a_train/prediction/prediction01.txt', active_users, '%s')
...
def predict1():
    ...
    np.savetxt('G:/dataset/a3d6_chusai_a_train/prediction/prediction01_1.txt', active_users, '%s')
