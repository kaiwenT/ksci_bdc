...
def cnn_model_fn(features, labels, mode):
    ...
    pool3_flat = tf.reshape(pool2, [-1, 1 * 21 * 256])
    # 全连接层FC1
    dense1 = tf.layers.dense(pool3_flat, units=2048, activation=tf.nn.tanh)
    # dropout1
...
def train():
    ...
    cifar_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir='G:/models/ksci_bdc_model/model06')
